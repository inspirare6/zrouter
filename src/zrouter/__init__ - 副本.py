...
class Router(RouterMixin, Blueprint):
    ...
    def add_resource(self, rule, resource_class):
        methods = [method for method in dir(resource_class) if callable(getattr(resource_class, method))]
        if 'get' in methods:
            endpoint = str(random.randint(10000000, 99999999))
            self.add_url_rule(rule, endpoint, self.wrap_func(getattr(resource_class, 'get')), methods=['GET'])
        if 'delete' in methods:
            endpoint = str(random.randint(10000000, 99999999))
            self.add_url_rule(rule, endpoint, self.wrap_func(getattr(resource_class, 'delete')), methods=['DELETE'])
        if 'post' in methods:
            endpoint = str(random.randint(10000000, 99999999))
            self.add_url_rule(rule, endpoint, self.wrap_func(getattr(resource_class, 'post')), methods=['POST'])

        if 'put' in methods:
            endpoint = str(random.randint(10000000, 99999999))
            self.add_url_rule(rule, endpoint, self.wrap_func(getattr(resource_class, 'put')), methods=['PUT'])

    def add(self, rule, open=False, direct=False, **options):
        def decorator(f):
            endpoint = options.pop("endpoint", None)
            self.add_url_rule(rule, endpoint, self.wrap_func(f, open=open, direct=direct), **options)
        return decorator
